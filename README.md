...
1. **ゲーム本体 `EliteMay/type-tower-a` の現在の関数名・変数名・状態を正とする**
2. 方針サイトと本体が食い違っている場合は、原則として**方針サイト側を本体に合わせて修正する**
3. **既存の自分たちのコードをそのまま残す**
4. 新しいCSSルールやJavaScript関数は、原則として**ファイルの一番後ろへ追加**
5. HTMLの特定要素内や既存関数内へ入れる必要がある場合は、`code.html` に**「大体何行目」「どの既存コードの直後 / 直前か」**を明記する
6. 行番号だけでは本体変更でずれるため、位置案内は次の3点をセットで書く
   - 対象ファイル
   - 大体の行番号
   - 本体に実在するコードを目印にした「このコードの直後 / 直前」
7. 既存処理の変更が必要な場合だけ、対象の関数・ブロックを**部分的に置換**
8. STEP例を使うためにSELECT背景動画、塔名CSS、GAME背景、入力判定など前STEPの実装を消さない
9. JSONへ問題を追加するときは、単純なファイル末尾追加ではなく配列の最後の `]` より前へ正しいカンマ付きで追加する
...
```text
js/game.js の60行目前後。
`judgeMessage.textContent='正解！ +1F';` の直後に追加。
```

または:

```text
js/main.js の末尾（約25行目以降）。
`showScreen('select');` の後ろへ追加。
```

本体が更新されて行番号が変わった場合は、**行番号よりも実在するコード目印を優先**します。
...
- `data/en-ja.json` : 英語→日本語の問題データあり。ただしゲームへは未接続
- `data/ja-en.json` : 日本語→英語の問題データあり。ただしゲームへは未接続
